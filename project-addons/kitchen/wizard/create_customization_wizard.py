...
class CustomizationWizard(models.TransientModel):
    ...
    order_id = fields.Many2one('sale.order',default=lambda self: self.env['sale.order'].browse(self.env.context.get('active_ids')))
    ...
    def _get_lines(self):
        ...
        for line in self.env['sale.order'].browse(self.env.context.get('active_ids')).order_line:
            ...
            if customize_all=='False':
                qty=self.env['stock.move'].search([('sale_line_id','=',line.id),('state','!=','cancel')],limit=1).product_uom_qty
                if qty>0:
    ...
    def action_create(self):
        ...
        for line in self.customization_line:
            qty = line.qty
            if qty < 0 :
                raise UserError(_("You can't create a customization with a quantity of less than zero of this product: %s") % line.sale_line_id.product_id.default_code)
            elif line.max_qty < qty:
                raise UserError(_("You can't create a customization with a bigger quantity of the product than what appears in the order: %s") % line.sale_line_id.product_id.default_code)
            elif qty>0:
                new_line = {
